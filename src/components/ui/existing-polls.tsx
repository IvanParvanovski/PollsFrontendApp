import { getPolls } from "@/queries/polls/get-polls";
import { Card } from "./card";
import { useEffect, useState } from "react";

export function ExistingPolls() {
  const [activeTab, setActiveTab] = useState("All");
  const [polls, setPolls] = useState([]); // stores the polls

  useEffect(() => {
    getPolls()
      .then((data) => {
        setPolls(data?.data || []); // <-- fix here!
      })
      .catch((err) => {
  ...
  return (
    <>
      ...
      <div className=" space-y-2 md:p-4 overflow-y-auto flex-1">
        {polls.length > 0 ? (
          polls.map((p) => <Card poll={p} />)
        ) : (
          <p>No polls found.</p>
        )}

        {/* {polls.length != 0 ? polls.map((p) => () : (<p>NO polls</p>)}
            {polls.map((p) => (
